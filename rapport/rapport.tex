...
\subsubsection{\texttt{postgesql}}
    ...
    dans /etc/postgresql/9.4/main/pg_hba.conf ajouter la ligne:
        host    all             all             146.185.151.186/32            md5
        (en deessous de: host    all             all             127.0.0.1/32            md5)
    dans vim /etc/postgresql/9.4/main/postgresql.conf:
        changer #listen_addresses = 'localhost' par listen_addresses = '*'
